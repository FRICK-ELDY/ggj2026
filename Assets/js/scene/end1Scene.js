...
export async function createEnd1Scene(canvas, container, onSceneChange, onConfigChange = null, initialConfigState = null) {
  ...
  function updateNovelLayout(width, height) {
    const scaledW = getScale(width, height) * VN_BASE_WIDTH;
    const scaledH = getScale(width, height) * VN_BASE_HEIGHT;
    vnMesh.scale.set(scaledW, scaledH, 1);
    const halfH = height / 2;
    const marginBottom = 16 * getScale(width, height);
  ...
  const lines = [
    { name: 'シスター', text: '（……そうかもしれない）' },
    {
      name: 'シスター',
      text: '（神様は、遠くから裁く存在じゃなくて、人が人をどう扱うかを黙って見ている存在なのかもしれない。）'
    },
    {
      name: 'シスター',
      text: '（もしそうなら、私が沈黙した瞬間も、見ていたのだろう。）'
    },
    {
      name: 'シスター',
      text: '「……はい。人と人の間に神様がいるのなら、私たちはもっと互いを恐れなければいけませんね」'
    },
    { name: '神父', text: '「ほう……それはどういう意味ですか？」' },
    {
      name: 'シスター',
      text: '（神様が近いということは、逃げ場がないということ。）'
    },
    {
      name: 'シスター',
      text: '（許しも、赦されない罪も、全部、人の手の中にある。）'
    },
    {
      name: 'シスター',
      text: '（私は信じたのではない。ただ――責任から逃げることを、やめただけ。）'
    }
  ];
